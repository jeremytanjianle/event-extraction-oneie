...
class Graph(object):
    ...
    def to_dict(self):
        ...
        entities = [[i, j, entity_itos[k], mention_itos[l], s] for (i, j, k), (_, _, l), s in itertools.zip_longest(self.entities, self.mentions, self.entity_scores)]
        triggers = [[i, j, trigger_itos[k], l] for (i, j, k), l in itertools.zip_longest(self.triggers, self.trigger_scores)]
        relations = [[i, j, relation_itos[k], l] for (i, j, k), l in itertools.zip_longest(self.relations, self.relation_scores)]
        roles = [[i, j, role_itos[k], l] for (i, j, k), l in itertools.zip_longest(self.roles, self.role_scores)]
